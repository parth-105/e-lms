    ...
    <div className="fixed inset-0">
      <div ref={containerRef} className="h-full flex flex-col bg-gray-800">
        {typeof localParticipantAllowedJoin === "boolean" ? (
          localParticipantAllowedJoin ? (
            <>
              <div className={`flex flex-1 ${isMobile || isTab ? 'flex-col' : 'flex-row'} bg-gray-800`}>
                <div className={`flex flex-1 ${isMobile || isTab ? 'flex-col' : 'flex-row'}`}>
                ...
                {/* Sidebar will be hidden on mobile and tablet */}
                {/* {!(isMobile || isTab) && (
                  <SidebarConatiner
                    height={containerHeight - bottomBarHeight}
                    sideBarContainerWidth={sideBarContainerWidth}
              ...
              {/* BottomBar should always be visible */}
              <BottomBar
                bottomBarHeight={bottomBarHeight}
                setIsMeetingLeft={setIsMeetingLeft}
              />
              ...
              {/* 
              {(isMobile || isTab) ? (
                <div className="fixed bottom-0 left-0 w-full z-50 bg-red rounded-t-2xl shadow-xl p-4 transition-transform transform translate-y-0">
                  <BottomBar setIsMeetingLeft={setIsMeetingLeft} />
                </div>
              ) : (
                <div style={{ height: bottomBarHeight }} className="w-full bg-white shadow-sm">
                  <BottomBar setIsMeetingLeft={setIsMeetingLeft} />
                </div>
              )} */}
